...
router.get("/from-to-lists", async (req, res) => {
  try {
    const fromList = await Bus.distinct("from");
    const toList = await Bus.distinct("to");

    res.status(200).send({
      success: true,
      message: "Fetched location lists",
      data: {
        fromList,
        toList,
      },
    ...
    res.status(500).send({
      success: false,
      message: "Error fetching location lists",
      error: error.message,
    });
  }
});
